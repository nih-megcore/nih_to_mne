...
def make_fast_head_surface(
    ...
    *,
    ...
    """Create a lightweight head FIF surface from FreeSurfer ``T1.mgz``.
    ...
    overwrite
        Replace ``bem/<subject>-head.fif`` when it already exists.
    """
    ...
    from mne import write_head_bem
    ...
    output = bem_dir / f"{subject}-head.fif"
    bem_dir.mkdir(parents=True, exist_ok=True)
    temp_fd, temp_name = tempfile.mkstemp(
        prefix=f".{subject}-head-", suffix=".fif", dir=bem_dir
    )
    os.close(temp_fd)
